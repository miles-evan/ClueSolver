...
This program is designed to take in information about what happens in a game of Clue, and determine what cards people have in their hands with a hybrid constraint-propagation and backtracking algorithm.
...
The ClueGame class takes user input to simulate a game by communicating with the Clue class.
...
The ClueBacktracker holds the same instance of Hands and ClueLogic as the Clue class does.
...
If it's impossible to set an entry to a check, for example, then the Clue class would set that entry to an X.
